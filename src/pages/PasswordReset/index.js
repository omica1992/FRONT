...
const ResetPasswordPage = () => {
  ...
  const handleResetPassword = async () => {
    try {
      ...
      const response = await api.get("/obter-usuarios");
      ...
      if (foundUser) {
        ...
        try {
          await api.post("/enviar-email", { email: foundUser.email });
  ...
  const handleResetPass = async () => {
    try {
      ...
      const { data: { userId } } = await api.post("/enviar-email", { wpp: email });
  ...
  const handleSavePassword = async () => {
    if (userFound && codeVerified) {
      try {
        ...
        await api.put("/atualizar-senha", { userId, newPassword });
